...
const BASE_COURSES = [
  {
    ...
    room:'Online · Zoom-style class (Demo)', schedule:'Every Tuesday · 6:00–8:30 PM · Online', units:4, color:'#2f6f55', grade:96,
    modality:'Synchronous Online', week:'Week 3',
    description:'A fictional introductory engineering course built as a local interactive demo. Topics include engineering design, quantitative reasoning, technical communication, statics concepts, teamwork, ethics, and decision-making.',
    ...
    assignments:[
      ...
      },
      {
        ...
        instructions:'Complete all four parts. You may type your reasoning below and optionally attach an image/PDF of hand-drawn diagrams. This is an original demo worksheet.',
        questions:[
    ...
    modules:[
      {title:'Start Here · Course Orientation',state:'complete',items:[
        {type:'page',title:'Welcome & How This Course Works',detail:'This fictional course uses weekly modules. Each module combines lecture notes, public learning resources, practice, graded work, and discussion.'},
        {type:'page',title:'Technology Check',detail:'Use a modern browser. Local demo submissions are stored only in this browser using localStorage; nothing is sent to De Anza, Canvas, Zoom, or an instructor.'},
      ...
      {title:'Week 4 · Sep 14–20 · Moments & Equilibrium',state:'upcoming',items:[
        {type:'page',title:'Week 4 Preview',detail:'Next week extends free-body diagrams into moment calculations and complete 2D equilibrium. Content unlocks in this demo on Sep 14.'},
        {type:'zoom',zoomId:'z4'}
      ]}
    ]
  },
  {
    id:'math1a',code:'MATH 1A',title:'Calculus',instructor:'Dr. Luis Ramirez',room:'Online · Asynchronous (Demo)',schedule:'Online · Weekly modules · No fixed meeting',units:5,color:'#356a8a',grade:92,modality:'Online Asynchronous',week:'Week 3',
    description:'A fictional calculus course covering limits, derivatives, applications, and introductory integration.',gradeGroups:[['Homework',35,92],['Quizzes',20,90],['Exams',40,90],['Participation',5,100]],
  ...
  {
    id:'phys4a',code:'PHYS 4A',title:'Physics for Scientists & Engineers: Mechanics',instructor:'Dr. Aisha Patel',room:'Lab 110 (Demo)',schedule:'Thursday · 2:00–5:15 PM · Hybrid',units:5,color:'#56805a',grade:93,modality:'Hybrid',week:'Week 3',
    description:'A fictional calculus-based mechanics course covering motion, vectors, force, energy, momentum, and rotation.',gradeGroups:[['Labs',30,95],['Homework',20,94],['Exams',45,90],['Participation',5,100]],lectures:{},
    assignments:[
  ...
  {
    id:'ewrt1a',code:'EWRT 1A',title:'Composition and Reading',instructor:'Prof. Jordan Lee',room:'Online · Zoom workshop (Demo)',schedule:'Online asynchronous + Thursday Zoom · 7:00–8:00 PM',units:5,color:'#7c5a8d',grade:97,modality:'Online + Optional Zoom',week:'Week 3',
    description:'A fictional composition course focused on college-level reading, argument, drafting, research, and revision.',gradeGroups:[['Essays',55,97],['Reading',20,95],['Discussion',15,100],['Participation',10,100]],lectures:{},
    assignments:[
...
const ANNOUNCEMENTS=[
  {id:'a1',course:'ENGR 10',title:'Week 3 · Tonight on Zoom',body:'Tonight’s fictional class runs 6:00–8:30 PM. Open Lecture 3 before class and bring paper for free-body diagrams. The exit ticket closes at 9:00 PM.',time:'Sep 8, 3:40 PM',target:{course:'engr10',tab:'zoom'}},
  {id:'a2',course:'ENGR 10',title:'Week 2 mini-project feedback posted',body:'Scores and fictional feedback are visible in Grades. Most submissions handled criteria well; uncertainty in cost estimates needs more attention.',time:'Sep 7, 6:18 PM',target:{course:'engr10',assignment:'engr-designbrief'}},
  ...
  {id:'a4',course:'EWRT 1A',title:'Optional Zoom draft workshop Thursday',body:'Bring one paragraph you want feedback on. The workshop is optional in this simulation.',time:'Sep 8, 11:22 AM',target:{course:'ewrt1a',tab:'zoom'}}
];
...
const MESSAGES=[
  ...
  {id:'m2',initials:'MC',from:'Prof. Maya Chen',subject:'Feedback posted',body:'Your Week 2 mini-project has fictional feedback in Grades. Nice work on the decision matrix. Review the comment about uncertainty before the next project.',time:'Yesterday',unread:true,course:'ENGR 10'},
...
function defaultState(){
  ...
  return {profile:{name:'Demo Student',photo:''},submissions,discussionPosts:{},quizChecks:{},courseOverrides:{},history:[],drafts:{},messagesRead:{},calendarMonth:'2026-09'};
}
...
function initials(name='Demo Student'){return name.trim().split(/\s+/).slice(0,2).map(x=>x[0]?.toUpperCase()||'').join('')||'DS';}
...
function setView(id){
  ...
  }else crumbs.innerHTML=`<span>De Anza College · Unofficial Demo</span><b>›</b><strong>${escapeHTML(id[0].toUpperCase()+id.slice(1))}</strong>`;
...
function renderProfile(){
  const name=state.profile.name||'Demo Student';
...
document.getElementById('saveProfileBtn').onclick=()=>{state.profile.name=document.getElementById('studentNameInput').value.trim()||'Demo Student';saveState();renderProfile();renderDashboard();toast('Profile saved locally');};
...
function renderAllCourses(){
  ...
  document.querySelectorAll('.small-side-panel button').forEach((b,i)=>b.onclick=()=>toast(i===0?'Showing active Fall 2026 demo courses':'No past enrollments in this local demo'));
}

function openCourse(id,tab='home'){
  currentCourse=courseById(id);currentCourseTab=tab;setView('courseDetail');
  document.getElementById('courseBreadcrumb').textContent=`${currentCourse.code} · Fall 2026 · Unofficial Demo`;
  selectCourseTab(tab);
...
function renderCourseTab(tab){
  ...
  if(tab==='home'){
    ...
    el.innerHTML=`<div class="course-home-grid"><section><div class="demo-badge">UNOFFICIAL LOCAL DEMO · WEEK 3</div><h2 class="content-heading">${escapeHTML(c.full)}</h2><p class="lead-copy">${escapeHTML(c.description)}</p><div class="course-meta-grid"><div><small>Instructor</small><strong>${escapeHTML(c.instructor)}</strong></div><div><small>Meeting</small><strong>${escapeHTML(c.schedule)}</strong></div><div><small>Delivery</small><strong>${escapeHTML(c.modality)}</strong></div><div><small>Current grade</small><strong>${c.grade}% · ${gradeLetter(c.grade)}</strong></div></div><div class="home-callout"><h3>${escapeHTML(currentModule.title)}</h3><p>${currentModule.items.length} published items in the current module.</p><button class="primary-btn" data-jump="modules">Go to current module</button></div><div class="home-block"><h3>Recent Announcements</h3>${ANNOUNCEMENTS.filter(a=>a.course===c.code).slice(0,3).map(a=>`<button class="announcement-mini" data-announcement="${a.id}"><strong>${escapeHTML(a.title)}</strong><small>${escapeHTML(a.time)}</small><span>${escapeHTML(a.body)}</span></button>`).join('')||'<p class="muted">No announcements.</p>'}</div></section><aside class="course-side"><button class="primary-btn full" data-jump="modules">View Course Modules</button><button class="outline-btn full" data-jump="assignments">Assignments</button><button class="outline-btn full" data-jump="grades">Grades</button><button class="outline-btn full" data-jump="zoom">Zoom</button><div class="side-note"><strong>Week 3</strong><span>Simulation date: Sep 8, 2026</span></div></aside></div>`;
  } else if(tab==='zoom'){
    const zs=c.zoomMeetings||[];
    el.innerHTML=zs.length?`<div class="section-intro"><h2>Class Meetings</h2><p>Meeting cards are interactive simulations. No real Zoom account or school meeting is connected.</p></div><div class="zoom-list">${zs.map(z=>`<article class="zoom-card ${z.status}"><div class="zoom-date"><strong>${escapeHTML(z.date)}</strong><span>${escapeHTML(z.time)}</span></div><div class="zoom-info"><h3>${escapeHTML(z.title)}</h3><p>${z.status==='recording'?'Completed · notes/recording simulation available':z.status==='today'?'Today · live meeting simulation':'Upcoming'}</p><button class="${z.status==='today'?'primary-btn':'outline-btn'} zoom-open" data-zoom="${z.id}">${z.status==='recording'?'Open recording notes':z.status==='today'?'Join class simulation':'View meeting details'}</button></div><div class="zoom-status">${z.status==='recording'?'Recorded':z.status==='today'?'TODAY':'Upcoming'}</div></article>`).join('')}</div>`:`<div class="empty-state"><h3>No recurring Zoom meetings</h3><p>This course is ${escapeHTML(c.modality)}.</p></div>`;
  } else if(tab==='announcements'){
    const aa=ANNOUNCEMENTS.filter(a=>a.course===c.code);
    el.innerHTML=`<div class="section-intro"><h2>Announcements</h2><p>Course updates published in this fictional demo.</p></div>${aa.map(a=>`<button class="announcement-row announcement-open" data-announcement="${a.id}"><h3>${escapeHTML(a.title)}</h3><small>${escapeHTML(a.time)} · ${escapeHTML(c.instructor)}</small><p>${escapeHTML(a.body)}</p></button>`).join('')||'<div class="empty-state"><h3>No announcements</h3></div>'}`;
    ...
    el.innerHTML=`<div class="section-intro"><h2>People</h2><p>All names on this page are fictional demo identities.</p></div><div class="people-grid">${people.map(p=>`<button class="person-row person-open" data-person="${escapeHTML(p.name)}" data-role="${p.role}"><div class="person-avatar">${initials(p.name)}</div><div><h4>${escapeHTML(p.name)}</h4><p>${p.role} · Fictional</p></div><div>${p.role}</div></button>`).join('')}</div>`;
  } else if(tab==='pages'){
    ...
    const files=[['Syllabus_Fall_2026_DEMO.txt','Course syllabus text'],['Week_01_Notes_DEMO.txt','Week 1 lecture summary'],['Week_02_Decision_Matrix_DEMO.csv','Example decision matrix'],['Week_03_FBD_Practice_DEMO.txt','Free-body-diagram practice prompts'],['Assignment_Rubrics_DEMO.txt','Rubric reference']];
    el.innerHTML=`<div class="section-intro"><h2>Course Files</h2><p>These are local demo files generated on demand; clicking Download creates a file in your browser.</p></div><div class="lms-list"><div class="lms-list-header">Files</div>${files.map((f,i)=>`<div class="lms-row"><div class="row-icon">▤</div><div><h4>${f[0]}</h4><p>${f[1]}</p></div><button class="outline-btn file-download" data-file="${i}">Download</button></div>`).join('')}</div>`;
  } else if(tab==='syllabus'){
    el.innerHTML=`<div class="syllabus-text"><div class="demo-badge">UNOFFICIAL DEMO SYLLABUS</div><h2 class="content-heading">${escapeHTML(c.full)}</h2><p><strong>${escapeHTML(c.instructor)}</strong><br>${escapeHTML(c.schedule)}<br>${escapeHTML(c.room)}<br>${c.units} units · ${escapeHTML(c.modality)}</p><h3>Current Simulation Point</h3><p><strong>Week 3 · September 7–13, 2026.</strong> Weeks 1 and 2 contain completed fictional work and instructor feedback.</p><h3>Course Description</h3><p>${escapeHTML(c.description)}</p><h3>Learning Outcomes</h3><ul><li>Apply course concepts to structured problems.</li><li>Communicate reasoning clearly and show supporting work.</li><li>Evaluate evidence, assumptions, and trade-offs.</li><li>Participate in synchronous or asynchronous course activities as configured.</li></ul><h3>Grading</h3><p>${c.gradeGroups.map(g=>`${escapeHTML(g[0])} ${g[1]}%`).join(' · ')}</p><h3>Demo Integrity Notice</h3><p>This page is part of a local fictional simulation. It is not an official De Anza College syllabus, enrollment, transcript, grade report, or Canvas page.</p></div>`;
  } else if(tab==='modules'){
    ...
    el.innerHTML=`<div class="settings-panel"><div class="demo-badge">LOCAL DEMO SETTINGS</div><h2>Course Details</h2><p>Changes apply only to this local browser demo.</p><div class="settings-form"><label>Course code<input id="courseCodeInput" value="${escapeHTML(c.code)}"></label><label>Course name<input id="courseTitleInput" value="${escapeHTML(c.title)}"></label><label>Instructor<input id="courseInstructorInput" value="${escapeHTML(c.instructor)}"></label><label>Room / delivery<input id="courseRoomInput" value="${escapeHTML(c.room)}"></label><label class="full-field">Meeting time<input id="courseScheduleInput" value="${escapeHTML(c.schedule)}"></label><div class="form-actions full-field"><button class="primary-btn" id="saveCourseSettings">Save Changes</button><button class="outline-btn" id="resetCourseSettings">Reset This Course</button></div></div></div>`;
  }
  bindCourseControls();
}
...
function openAssignment(c,a){
  ...
  const renderTab=tab=>{
    ...
    if(tab==='details') body.innerHTML=`<div class="rich-copy"><h3>Instructions</h3><p>${escapeHTML(a.instructions||'Complete the questions below and submit your responses.')}</p><h3>Work</h3><div class="assignment-work">${a.questions.map((q,i)=>renderQuestion(q,i,answers[i])).join('')}</div>${!submitted?'<button class="primary-btn" id="saveAssignmentDraft">Save Draft</button>':'<p class="muted">This submitted demo attempt is shown in the Submission tab.</p>'}</div>`;
...
function discussionComposer(c,d,parentIndex){
  const name=state.profile.name||'Demo Student';showModal(`<div class="modal-kicker">${parentIndex==null?'Reply to Topic':'Reply to Post'}</div><h2>${escapeHTML(d.title)}</h2><label class="assignment-question"><strong>Posting as ${escapeHTML(name)}</strong><textarea id="discussionText" placeholder="Write your response…"></textarea></label><button class="primary-btn" id="postDiscussionReply">Post Reply</button>`);
...
function openMeetingRoom(c,z){
  ...
  document.getElementById('handDemo').onclick=e=>{e.currentTarget.classList.toggle('active-control');e.currentTarget.textContent=e.currentTarget.classList.contains('active-control')?'✋ Hand Raised':'✋ Raise Hand';toast(e.currentTarget.classList.contains('active-control')?'Hand raised in demo':'Hand lowered');};
  ...
  document.getElementById('leaveDemo').onclick=()=>{closeModal();toast('Left class simulation');};
}
...
function downloadDemoFile(index){
  const c=currentCourse;const contents=[
    `UNOFFICIAL FICTIONAL DEMO\n${c.full}\nInstructor: ${c.instructor}\nMeeting: ${c.schedule}\n\nThis is not an official De Anza College document.`,
    ...
    `WEEK 3 FBD PRACTICE - DEMO\n1) 1200-lb midspan beam on two supports. Find reactions.\n2) Trace a floor load to soil.\n3) Describe the FBD of a wall shelf.`,
    `DEMO RUBRIC REFERENCE\nShow reasoning. Label units. Identify assumptions. Use clear diagrams. Explain trade-offs.\nNot an official De Anza College rubric.`
  ];
  const names=['Syllabus_Fall_2026_DEMO.txt','Week_01_Notes_DEMO.txt','Week_02_Decision_Matrix_DEMO.csv','Week_03_FBD_Practice_DEMO.txt','Assignment_Rubrics_DEMO.txt'];const blob=new Blob([contents[index]],{type:'text/plain'});const u=URL.createObjectURL(blob);const a=document.createElement('a');a.href=u;a.download=names[index];a.click();URL.revokeObjectURL(u);toast('Demo file created');
}
...
function composeMessage(replyTo=null){showModal(`<div class="modal-kicker">Compose Demo Message</div><h2>${replyTo?'Reply':'New Message'}</h2><div class="compose-form"><label>To<input id="composeTo" value="${escapeHTML(replyTo?.from||'')}"></label><label>Subject<input id="composeSubject" value="${escapeHTML(replyTo?`Re: ${replyTo.subject}`:'')}"></label><label>Message<textarea id="composeBody" placeholder="Type a local demo message…"></textarea></label><button class="primary-btn" id="sendCompose">Send Demo Message</button></div>`);document.getElementById('sendCompose').onclick=()=>{if(!document.getElementById('composeBody').value.trim())return toast('Write a message first');toast('Demo message sent locally');closeModal();};}
document.querySelector('.inbox-title .primary-btn').onclick=()=>composeMessage();
document.querySelectorAll('.inbox-toolbar .outline-btn').forEach((b,i)=>b.onclick=()=>toast(i===2?'Inbox refreshed':'Filter applied to demo inbox'));
...
function openSearch(){
  ...
  showModal(`<div class="modal-kicker">Global Search</div><h2>Search this demo</h2><input id="globalSearchInput" class="search-input" autofocus placeholder="Try “free body”, “homework”, “ENGR 10”…"><div id="globalSearchResults" class="search-results"></div>`);
...
document.querySelectorAll('#help .help-box').forEach((box,i)=>box.onclick=()=>{if(i===0)showModal('<div class="modal-kicker">Demo Guide</div><h2>How to explore V6</h2><ol class="guide-list"><li>Open ENGR 10 → Study Center to review prior work and lectures.</li><li>Open CONST 20 → Modules and try the drawing, concrete, and room-takeoff labs.</li><li>Open any completed assignment to inspect fictional submitted work and instructor feedback.</li><li>Retry lecture knowledge checks and use flashcards/private notes.</li><li>Open a discussion and add a local reply, then join a Zoom-style class simulation.</li><li>Use global Search, Calendar, Inbox, Files, and History to explore the semester.</li></ol>');if(i===1)setView('account');if(i===2)showModal('<div class="modal-kicker">Important</div><h2>Unofficial fictional demo</h2><p>This local website is not affiliated with De Anza College, FHDA, Canvas, Instructure, or Zoom. It must not be used as proof of enrollment, attendance, grades, or course participation.</p>');});
